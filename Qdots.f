...
4000    CONTINUE
        ...
        DO 5002 IB=1,NEF
        AEF=AEFI+(IB-1)*DEF
c      AEF=-4.394160033584097*DELTA+6.406232527887537*T
c        AEF=-41.3*DELTA
        EF=AEF
        EF1=EF
...
c       B=-41.27*DELTA
c      NTT=1000000
c      PESFILE="PES+T"//trim(Tconsole)//"ONLYE=-41.3.dat"
c      CALL GRAF(A,B,NTT,PESFILE)
...
c      PESFILE="PES-T"//trim(Tconsole)//"E=-41.3.dat"
c      CALL GRAF(A,B,NTT,PESFILE)
...
*****************************************
        ...
        A=DIMAG(ZGCON(EW,ETA))
        B=DREAL(ZGCON(EW,ETA))
        FER=FERM(EW)
	DFER=FER*(1.D0-FER)*BETA 
...
c     Código original:
        ZW=DCMPLX(BW,ETA)
        TRA=DIMAG(ZGLOCAL(BW,ETA))*DIMAG(ZGLOCAL(BW,ETA))
        TRA=TRA+DREAL(ZGLOCAL(BW,ETA))*DREAL(ZGLOCAL(BW,ETA))
        TRA=TRA*GAM
        R0F1=-CC*DIMAG(ZGLOCAL(BW,ETA))
        R0F1=DABS(R0F1)
        R0F2=CC*CC*0.25D0*R0F1*FERM(BW)
        R0F3=CC*CC*0.25D0*(1.0D0-FERM(BW))*R0F1
        R0F4=-CC*DIMAG(ZGCON(BW,ETA))
        R0F4=DABS(R0F4)
...
c        A0W=I*DELTA*1.0d-3
c        GIMLOC=DIMAG(ZGLOCAL(BW,AW))
c        GREALOC=DREAL(ZGLOCAL(BW,AW))
c        TRALOC=(GIMLOC*GIMLOC+GREALOC*GREALOC)*GAM
c        GIMCON=DIMAG(ZGCON(BW,AW))
c        GREALCON=DREAL(ZGCON(BW,AW))
c        TRACON=(GIMCON*GIMCON+GREALCON*GREALCON)*GAM
*************************************
...
c      WRITE(DT81,5800)BW*1.0d2,AW*1.0d2,GIMLOC,GREALOC,TRALOC,GIMCON,
c     *GREALCON,TRACON,GAM
c90     CONTINUE
80      CONTINUE
5800    FORMAT(8F15.8)
        CLOSE(DT81)
